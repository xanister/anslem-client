 ...
 */
define(['AnslemClientConfig', 'NodeClient', 'Sprite', 'Stage', 'howler'], function (AnslemClientConfig, NodeClient, Sprite, Stage) {
    /**
            ...
            for (var index in self.data.packet.inView) {
                ...
                if (e.sprite.tileX)
                    for (var xx = -Math.floor((self.data.packet.viewX * e.sprite.scrollSpeed) % e.width); xx < self.stage.canvas.width; xx = xx + e.width) {
                        ...
                        sprite.drawExt(ctx, e.sprite.frame, xx, e.y - (e.height / 2) - self.data.packet.viewY, e.width, e.height, 0, 0, e.sprite.width, e.sprite.height, e.sprite.mirror);
                    }
                else
